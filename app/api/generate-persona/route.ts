...
export async function POST(request: Request) {
  try {
    ...
    const baseFile = await toFile(Buffer.from(baseB64, "base64"), "base.png", { type: "image/png" });
    ...
    return NextResponse.json({
      closed: `data:image/png;base64,${baseB64}`,
      smile: `data:image/png;base64,${smileB64}`,
      talk: `data:image/png;base64,${talkB64}`,
    });
  } catch (error) {
